...
const PATIENT_CHECK_MODES = {
  round: {
    ...
    toggleOnTitle: '回診チェック表示中：未回診が上に並びます',
    toggleOffTitle: '回診チェック：回った患者に✓をつけて回り残しを見える化'
  },
  chart: {
    icon: '📝',
    label: 'カルテ',
    dateField: 'chartedDate',
    atField: 'chartedAt',
    ...
    toggleOnTitle: 'カルテチェック表示中：未記載が上に並びます',
    toggleOffTitle: 'カルテチェック：書いた患者に✓をつけて書き残しを見える化'
  }
};
const isCheckedToday = (p, kind) => {
  const meta = PATIENT_CHECK_MODES[kind];
  return !!(p && meta) && p[meta.dateField] === todayStr();
};
function CheckPanel({
  mode,
  summary,
  ...
  const meta = PATIENT_CHECK_MODES[mode] || PATIENT_CHECK_MODES.round;
  ...
  }, `${meta.icon} ${meta.label}チェック`), React.createElement("span", {
    style: {
      fontSize: 12,
      fontWeight: 800,
  ...
  }, meta.allDoneMsg) : React.createElement("div", {
    style: {
      fontSize: 11,
      color: 'var(--text-3)',
      fontWeight: 600
...
function PatientCard({
  ...
  return React.createElement("div", {
    ...
    style: {
      ...
      alignItems: 'center',
      gap: 4,
      background: checked ? 'rgba(22,163,74,.12)' : 'transparent',
      border: checked ? '1.5px solid rgba(22,163,74,.45)' : '1.5px dashed var(--border-2)',
      color: checked ? '#15803D' : 'var(--text-3)',
      borderRadius: 9,
      cursor: 'pointer',
      padding: '4px 7px',
      fontSize: 11,
  ...
  }, checked ? `\u2713 ${checkMeta.label}` : `${checkMeta.icon} ${checkMeta.label}`, checked && checkedAt && React.createElement("span", {
    className: "round-stamp-time"
  }, ' ' + formatHHMM(checkedAt))), !showCheckStamp && React.createElement("button", {
    onClick: e => {
      e.stopPropagation();
      onRemove();
    },
...
function TemplatesSection({
  ...
  return React.createElement("div", {
  ...
  }, templates.map((tpl, tplIndex) => React.createElement("div", {
    key: tpl.id,
    style: {
      background: 'var(--surface-2)',
      borderRadius: 10,
    ...
    onClick: () => onMoveTemplate(tpl.id, -1),
    className: "btn-sm",
    disabled: tplIndex === 0,
    title: "セットを上へ",
    "aria-label": `${tpl.name || 'セット'}を上へ`,
    style: {
      padding: '4px 7px',
      opacity: tplIndex === 0 ? .3 : 1
    }
  }, "↑"), React.createElement("button", {
    onClick: () => onMoveTemplate(tpl.id, 1),
    className: "btn-sm",
    disabled: tplIndex === templates.length - 1,
    title: "セットを下へ",
    "aria-label": `${tpl.name || 'セット'}を下へ`,
    style: {
      padding: '4px 7px',
      opacity: tplIndex === templates.length - 1 ? .3 : 1
    }
  }, "↓"), React.createElement("button", {
    ...
    onClick: () => onMoveItem(tpl.id, idx, -1),
    className: "btn-sm",
    disabled: idx === 0,
    title: "項目を上へ",
    "aria-label": `${item.title || '項目'}を上へ`,
    style: {
      padding: '3px 6px',
      opacity: idx === 0 ? .3 : 1
    }
    ...
    title: "項目を下へ",
    "aria-label": `${item.title || '項目'}を下へ`,
    style: {
      padding: '3px 6px',
      opacity: idx === tpl.items.length - 1 ? .3 : 1
    }
  }, "↓"), React.createElement("button", {
...
function DailyTaskSetManager({
  ...
  return React.createElement("div", {
  ...
  }, "+ セット追加"), sets.map((set, setIndex) => React.createElement("div", {
    key: set.id,
    style: {
      background: 'var(--surface-2)',
      border: '1px solid var(--border)',
    ...
    style: { ...inpStyle, flex: '1 1 150px', minWidth: 0, fontWeight: 800 },
    "aria-label": "セット名"
  }), React.createElement("button", {
    className: "btn-sm",
    onClick: () => onMoveSet(set.id, -1),
    disabled: setIndex === 0,
    title: "セットを上へ",
    "aria-label": `${set.name || 'セット'}を上へ`,
    style: { padding: '4px 7px', opacity: setIndex === 0 ? .3 : 1 }
  }, "↑"), React.createElement("button", {
    className: "btn-sm",
    onClick: () => onMoveSet(set.id, 1),
    disabled: setIndex === sets.length - 1,
    title: "セットを下へ",
    "aria-label": `${set.name || 'セット'}を下へ`,
    style: { padding: '4px 7px', opacity: setIndex === sets.length - 1 ? .3 : 1 }
  }, "↓"), React.createElement("button", {
    className: "btn-sm",
    ...
    style: {
      display: 'grid',
      gridTemplateColumns: 'minmax(0,1.4fr) minmax(68px,.65fr) minmax(60px,.5fr) minmax(60px,.5fr)',
      gap: 5,
      alignItems: 'center'
    }
  }, React.createElement("input", {
    ...
    style: inpStyle
  }, DAILY_TASK_PRIORITIES.map(p => React.createElement("option", { key: p.id, value: p.id }, p.label))), React.createElement("div", {
    style: { display: 'flex', gap: 3, alignItems: 'center', justifyContent: 'flex-end', gridColumn: '1 / -1' }
  }, React.createElement("button", {
    className: "btn-sm",
    onClick: () => onMoveItem(set.id, idx, -1),
    disabled: idx === 0,
    title: "項目を上へ",
    "aria-label": `${item.title || '項目'}を上へ`,
    style: { padding: '3px 6px', opacity: idx === 0 ? .3 : 1 }
  }, "↑"), React.createElement("button", {
    className: "btn-sm",
    onClick: () => onMoveItem(set.id, idx, 1),
    disabled: idx === (set.items || []).length - 1,
    title: "項目を下へ",
    "aria-label": `${item.title || '項目'}を下へ`,
    style: { padding: '3px 6px', opacity: idx === (set.items || []).length - 1 ? .3 : 1 }
  }, "↓"), React.createElement("button", {
    className: "btn-sm",
...
function PatientTriage() {
  ...
  const checkedCounts = useMemo(() => ({
    ...
    chart: checkTargets.filter(p => isCheckedToday(p, 'chart')).length
  }), [checkTargets, now]);
  const checkWardSummary = useMemo(() => {
    if (!checkMode) return [];
    const map = new Map();
    checkTargets.forEach(p => {
      ...
      entry.total += 1;
      if (isCheckedToday(p, checkMode)) entry.done += 1;
    });
    return [...map.values()].sort((a, b) => (WARD_ORDER[a.ward] ?? 0) - (WARD_ORDER[b.ward] ?? 0));
  }, [checkTargets, checkMode, now]);
  const displayPatients = useMemo(() => checkMode && !isDailyMode ? [...visiblePatients].sort((a, b) => (isCheckedToday(a, checkMode) ? 1 : 0) - (isCheckedToday(b, checkMode) ? 1 : 0)) : visiblePatients, [visiblePatients, checkMode, isDailyMode, now]);
  ...
  const toggleChecked = (kind, id) => {
    ...
    if (!target) return;
    const marking = !isCheckedToday(target, kind);
    setActivePatients(prev => prev.map(p => p.id === id ? marking ? {
      ...p,
      [meta.dateField]: todayStr(),
    ...
    if (marking) {
      const remaining = activePatients.filter(p => p.id !== id && isRoundTarget(p) && !isCheckedToday(p, kind)).length;
      if (remaining === 0) {
        showToast(meta.completeToast);
        window.dispatchEvent(new CustomEvent('chibi-coach', {
          detail: {
            kind: 'allclear',
            estimate: {
  ...
  const resetChecks = kind => {
    const meta = PATIENT_CHECK_MODES[kind];
    if (!meta) return;
    rememberUndo(`${meta.label}チェックリセット`);
    setActivePatients(prev => prev.map(p => p[meta.dateField] || p[meta.atField] ? {
      ...p,
      [meta.dateField]: null,
      [meta.atField]: null
    } : p));
    showToast(`${meta.label}チェックをリセットしました`);
  };
  ...
  const moveTemplate = (id, direction) => {
    rememberUndo('ぺいとりセット並べ替え');
    setTemplates(prev => {
      const fromIndex = prev.findIndex(template => template.id === id);
      return moveArrayItem(prev, fromIndex, fromIndex + direction);
    });
  };
  const moveTemplateItem = (id, index, direction) => {
    rememberUndo('ぺいとりセット項目並べ替え');
    setTemplates(prev => prev.map(template => template.id === id ? {
      ...template,
      items: moveArrayItem(template.items || [], index, index + direction)
    } : template));
  };
  ...
  const moveDailyTaskSet = (id, direction) => {
    rememberUndo('でいとりセット並べ替え');
    setDailyTaskSets(prev => {
      const fromIndex = prev.findIndex(set => set.id === id);
      return moveArrayItem(prev, fromIndex, fromIndex + direction);
    });
  };
  const moveDailyTaskSetItem = (id, index, direction) => {
    rememberUndo('でいとりセット項目並べ替え');
    setDailyTaskSets(prev => prev.map(set => set.id === id ? {
      ...set,
      items: moveArrayItem(set.items || [], index, index + direction)
    } : set));
  };
  ...
  }, "ER"), !isDailyMode && ['round', 'chart'].map(kind => {
    const meta = PATIENT_CHECK_MODES[kind];
    const active = checkMode === kind;
    return React.createElement("button", {
      key: kind,
      className: `btn-ghost${active ? ' btn-ghost-active' : ''}`,
      onClick: () => setCheckMode(m => m === kind ? '' : kind),
      "aria-label": `${meta.label}チェック`,
      "aria-pressed": active,
      title: active ? meta.toggleOnTitle : meta.toggleOffTitle,
      style: {
        whiteSpace: 'nowrap',
        flexShrink: 0,
        ...filterButtonStyle(active)
      }
    }, `${meta.icon} `, React.createElement("span", {
      className: "round-dock-label"
    }, `${meta.label} `), `${checkedCounts[kind]}/${checkTargets.length}`);
  })), React.createElement("div", {
  ...
  }, !isDailyMode && checkMode && React.createElement(CheckPanel, {
    mode: checkMode,
    summary: checkWardSummary,
